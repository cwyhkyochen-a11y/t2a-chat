// T2A Chat — 核心逻辑
// 职责：消息流、conversation CRUD、登录、WS 管理集成、DOM 渲染
...
(function () {
  ...
  function handleAuth(data) {
    if (data.success) {
      ...
      // 加载 UI 配置 + models
      if (window._t2aSlots) {
        window._t2aSlots.loadUiConfig(API_BASE);
      }
      _loadModels();
    } else {
  ...
  async function _loadModels(taskType) {
    try {
      ...
      console.warn('[t2aChat] loadModels failed:', e);
    }
  }
  ...
  async function apiCancelTask(taskId) {
    try {
      ...
      console.error('[t2aChat] cancelTask error:', e);
      toast('取消失败', 'error');
    }
  }
  ...
  async function apiCreateTask(type, params) {
    try {
      ...
      console.error('[t2aChat] createTask error:', e);
      toast('创建任务失败', 'error');
    }
  }
  ...
  async function apiGetTaskStatus(taskId) {
    try {
      ...
      console.error('[t2aChat] getTaskStatus error:', e);
    }
    return null;
  }
  ...
  window._t2aCore = {
    ...
    getTaskStatus: apiGetTaskStatus,
  };
  ...
  // 监听 suggestion 点击（来自 slots）
  if (window._t2aSlots) {
    window._t2aSlots.on('suggestion:clicked', function (data) {
      if (data.text) sendMessage(data.text);
    });
    // 监听 task 取消请求
    window._t2aSlots.on('task:cancel-request', function (data) {
      if (data.taskId) apiCancelTask(data.taskId);
    });
  }
})();
